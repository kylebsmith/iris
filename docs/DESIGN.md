...
**Verification artifacts:** a scratch directory that was not retained, `num2.c`, `num3.c` — the bandwidth/kernel/jitter sweeps, the Kahan comparison, the ladder-blend exactness proof, the activation benchmarks, and the file-format round-trip. `num2.c` is the one that should become `tests/num_probe.c`.
