/* sqrt_exhaustive.c -- iris_sqrt against the host's square root, for every
   one of the 2^32 float bit patterns.
   ...
   0xFFC00000), so for those patterns the check is that iris_sqrt returns a
   NaN.
...
int main(void) {
  ...
  do {
    ...
    const uint32_t got = bits_of(iris_sqrt(x));
    ...
    if (!agrees) {
      if (bad < 10)
        printf("  input 0x%08X: host 0x%08X, iris_sqrt 0x%08X\n",
  ...
  printf(bad ? "SQUARE ROOT DISAGREES\n" : "iris_sqrt is the host's square root, everywhere\n");
  return bad != 0;
}
