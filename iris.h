...
/* ============================================================================
   ...
   the end of this block. README.md introduces the library;
   docs/SYSTEM-technical.md describes the design for a technical reader and
   docs/SYSTEM-plain-english.md explains it from scratch.
   ...
   Three rules
     1. No allocation. You give it one block of memory, the arena, and it
        never asks for more, so its memory use is known before it runs.
     2. No C library. No printf, no math.h: the square root, the
        nonlinearity and the checksum are written out in this file.
     3. Single precision on the playing path. The ESP32-S3 does float
        arithmetic in hardware and double in slow software. The one use of
        double is the leave-one-out sweep (PART 8), a diagnostic that never
        runs while you play, which sums its squared misses in double so the
   ...
   function, built -std=c99 -ffreestanding -fno-stack-protector (and on GCC
   -fno-tree-loop-distribute-patterns) at -O0 to -O3 and -Os, as C or C++,
   ...
   tests/audit.c (2 inputs, 12 hidden units, 3 outputs) that is 18,000
   epochs: about 31 ms on the development laptop (`sh build.sh audit`
   prints it) and 13.4 s on an ES3C28P board (ESP32-S3, 240 MHz), measured
   by the starter kit's board_probe sketch, log in
   docs/board/2026-09-25-es3c28p.txt. README.md, "How fast it is",
   tabulates the rest. A sketch that must keep drawing trains in slices
   (iris_train_begin, iris_train_slice), bit-identical to iris_train.
   ...
   Words this file uses, defined once, here.
     ...
     activation: what a unit outputs after its squashing function; also the
       squashing function itself.
     arena: the one block of memory you give iris_init, sized by IRIS_ARENA.
     binary32, binary64: the 32- and 64-bit formats of IEEE 754, the
       floating-point standard of the Institute of Electrical and Electronics
       Engineers; C's float and double. A float has a sign, an 8-bit exponent
       and 24 significant bits, about seven decimal digits.
     bit-identical: equal in every bit, not merely close.
     C99, ISO C: the 1999 C standard; "ISO C" is a compiler mode without GNU
       extensions (-std=c99, not -std=gnu99).
     Cholesky: a fast solve of a symmetric linear system, by factoring its
       matrix into a triangle times that triangle's mirror image (PART 8d).
     clang, GCC: the two compiler families the tests use. GCC is the GNU
       Compiler Collection, and its -std=gnu... settings ("GNU mode") are
       what the Arduino build uses; the ESP32-S3, Cortex-M and AVR toolchains
       are GCC. Apple clang is the clang that ships with macOS.
     CRC-32: a cyclic redundancy check, the 32-bit checksum of a saved file;
       it catches accidental corruption, not deliberate editing (PART 9).
     ...
     error: how far the outputs are from the demonstrated ones. Mean squared
       error averages the squared misses; root-mean-square error is its root.
       Training errors are in the network's output units, where each
       output's demonstrated range spans 0.1 to 0.9 (PART 5).
     FNV-1a: the Fowler-Noll-Vo hash, byte by byte. A golden hash is the
       FNV-1a hash of what a fixed recipe produces, which the tests compare,
       so any change to the arithmetic fails a check (tests/audit.c pins
       0x6805FB0D).
     fused multiply-add: one instruction computing a*b + c with a single
       rounding. It changes the last bit, so this file forbids it in its own
       code; contraction is the compiler making one.
     geometric mean: the average of ratios taken by multiplying them and
       taking the root, so twice as good and twice as bad cancel.
     gradient: how much the error changes as each weight moves a little;
       training steps every weight against it, downhill (PART 8).
     held-out error: the error at points the instrument was not trained on,
       against the true target: how well it fills the gaps, which is what you
       play. Recall is the error at the demonstrations themselves; good
       recall with poor held-out error means the noise was fitted.
     IDE: integrated development environment, such as the Arduino IDE.
     k-NN, 1-NN: k-nearest-neighbour, a blend of the k stored demonstrations
       nearest a gesture; 1-NN returns the single nearest (PART 10).
     leave-one-out: hide one demonstration, train on the rest, measure the
       miss on the hidden one; repeat for each and average.
     learning rate, momentum: the size of each training step, and how much
       of the last step it carries on with; internal, 0.10 and 0.85.
     MLP: multilayer perceptron, the network of PARTS 6 and 8.
     NaN: not-a-number, the result of 0/0 or infinity minus infinity. Every
       comparison with it is false and it spreads through any arithmetic.
     NIME: the International Conference on New Interfaces for Musical
       Expression. "Fiebrink and Sonami, NIME 2020" is Rebecca Fiebrink and
       Laetitia Sonami, "Reflections on Eight Years of Instrument Creation
       with Machine Learning", 2020 proceedings, pages 237-242.
     normalised: on a fixed scale, inputs -1..+1 and outputs 0.1..0.9 across
       their demonstrated ranges (PART 5).
     ODR: the one-definition rule, that C and C++ require a thing to be
     ...
     saturate: a squashing function saturates where its output sits at or
       near its limit and a small change of input no longer moves it.
     seed: the number the random starting weights are drawn from; a new seed
       is a reroll. A seed of 0 is taken as 1.
     sigmoid, tanh: the S-shaped squashing functions (PART 1); tanh runs from
       -1 to +1, the sigmoid from 0 to 1.
     subnormal: a float so near zero that it gives up precision (denormal);
       some processors flush them to zero.
     translation unit: one .c file with everything it includes.
     ulp: unit in the last place, the gap between a float and the next.
     weight, bias: what the network learns; a weight scales a connection, a
       bias shifts a unit's sum.
     Weka, Wekinator: Weka is a Java machine-learning toolkit. Wekinator,
       Rebecca Fiebrink's tool for building instruments from demonstrations,
       trains Weka's MultilayerPerceptron and nearest-neighbour classifier;
       iris rebuilds that loop for boards with no operating system.

   Hardware named in comments: Arduino, the boards (and IDE) most students
   start with; the ESP32 family and its ESP32-S3, the Espressif
   microcontroller of the starter kit, with an Xtensa LX7 core; AVR, the
   8-bit processor of the Arduino Uno and Mega; RP2040 and STM32, other
   32-bit microcontrollers; ARM, the processors of phones, Apple-silicon Macs
   and the Raspberry Pi, whose Cortex-M cores are microcontrollers; x86 and
   x86-64, most other laptops, and x87, the old floating-point unit of 32-bit
...
/* ============================================================================
   ...
   Every public function, type and macro, one line each. `k` is the
   instrument. `in` and `out` are plain float arrays you own: `in` holds n_in
   floats and `out` n_out, and nothing checks their length, so an array
   ...
   every function is declared; and IRIS_NO_GUARDS, which compiles the guards
   out and is for measuring them, not for instruments. Every other IRIS_
   macro is part of how the file works, like the iris_internal_ names: it
   can change in any release, and defining one yourself is not supported.

   Failure, in two rules. A call that either works or does not returns 0 for
   "did nothing". A call that returns a measurement returns it, or -1 if it
   refused. iris_get_status answers a different question, whether the
   instrument is in trouble, and there 0 is good news (IRIS_STATUS_OK). The
   full account, with the one test that answers "did it train?" after every
   trainer, is above iris_get_status.
   ...
   whatever you give it, so scaling, centring or normalising them first
   gains nothing, and is one more thing to repeat exactly at play time.
   ========================================================================= */
...
/* --------------------------------------------------------------------------
   ...
   1. Tripwires for the flags that change the arithmetic. -ffast-math implies
      contract=fast and removes the not-a-number semantics the guards depend
      on, so it refuses to compile.                                         */
...
#endif
/* The component flags of -ffast-math also work alone. -freciprocal-math,
   -funsafe-math-optimizations and -fassociative-math each change the
   instrument with no diagnostic of any kind. GCC announces them with macros
   and clang does not, so this catches them on GCC only, the compiler of
   every ESP32, AVR and RP2040 build. No Arduino core passes any of them
   (platform.txt checked for arduino:avr, esp32:esp32, rp2040:rp2040 and
   STMicroelectronics:stm32), so reaching these #errors takes a deliberate
   flag.

   What this cannot catch: -fassociative-math passed on its own defines no
   macro on gcc-15 (gcc-15 -dM -E lists none), and clang defines none of
   these macros at all; clang 22's -ffp-model=fast defines only
   __FINITE_MATH_ONLY__, as 0, so it compiles without a word and moves the
   golden hash in tests/audit.c. Do not build with them. */
...
/* 2. Wider intermediate results. C lets a compiler carry float arithmetic in
      a wider format and round only when a value is stored;
      __FLT_EVAL_METHOD__ (FLT_EVAL_METHOD in <float.h>) says which format:

        0        every operation rounds to its own type: the arithmetic this
                 file is pinned to.
        16, 32   the same as 0 for float; only the half-precision type
                 ...
                 tests/audit.c built that way with gcc-15 gives its golden
                 hash.
      ...
      that this fires on x87 and nowhere else this library is built for.  */
...
#endif
/* 3. No contraction in this file's code, and only there.

      Clang honours #pragma STDC FP_CONTRACT OFF at its default and at
      -ffp-contract=on, where the instrument is bit-identical to a
      -ffp-contract=off build. It ignores the pragma under
      -ffp-contract=fast, so a -ffp-contract=fast clang build must also pass
      -ffp-contract=off.
      ...
      tests/audit.c holds with it and moves without it, and the ESP32-S3
      compiler emits no fused instruction in this file with it and dozens
      without it (tests/pragma_leak.sh prints the count).
      ...
      On clang, float_control(push) saves the floating-point state and
      float_control(pop) at the end restores it. Just before that pop,
      STDC FP_CONTRACT DEFAULT hands contraction back to the command line,
      because clang honours float_control only on processors it supports
      strict floating point for (64-bit ARM, x86, RISC-V and PowerPC among
      them) and ignores it on the rest (32-bit ARM, WebAssembly, Xtensa and
      AVR among them, with clang 22), with a warning the diagnostic lines
      around it keep out of your build. So code after the #include compiles
      as it would without iris.h, contraction included where the compiler's
      default allows it, with one exception: on those other clang targets a
      contraction pragma of your own placed before the #include gives way to
      the command line's setting, so put yours after it. tests/pragma_leak.sh
      checks this in the generated assembly: a*b+c in a function after the
      include still becomes a fused multiply-add, no iris function contains
      one, and on a laptop your own pragma before the include survives it.
      ...
      code contracts, your calls into iris stay calls (gcc-15), which keeps
      iris's arithmetic unfused inside your functions. And your own
      arithmetic is yours: a program that computes its demonstrations itself
      and needs the same bits from two compilers (a laptop and a board, say)
      switches contraction off in its own code too, as tests/audit.c does. */
...
/* The maxima size every working array, so on a small board they are the
   ...
   Working arrays are sized by these numbers, not by the shape you asked for:
   iris_internal_train_run alone reserves float x[IRIS_MAX_IN] and float
   t[IRIS_MAX_OUT], 192 bytes, whether the instrument has 32 inputs or 2.
   avr-gcc 7.3.0 -mmcu=atmega328p -Os -fstack-usage, with every function
   compiled on its own (IRIS_API defined empty), gives
   iris_internal_train_run a frame of 284 bytes and iris_predict 158. An Uno
   has 2 KB of memory in total and a sketch leaves a few hundred bytes of it
   for the stack, so the defaults do not fit it with room to spare.

   They are #ifndef so you can shrink them. Define them before including
   this file and every working array shrinks with them:
   ...
   frame on the record/train/predict path, falls from 284 bytes to 124, and
...
/* The cap keeps the size arithmetic inside 32 bits.
   ...
   The bound stops an overflow; 4,096 is not musically special. IRIS_ARENA
   and iris_size multiply cap by (n_in + n_out) and by sizeof(float) in
   unsigned long, which C guarantees is at least 32 bits (see IRIS_ARENA),
   and an uncapped count could wrap that product, so that the arena check
   passed a buffer too small and the demonstration store ran off its end. At
   the maxima (32 in, 64 hidden, 16 out) 4,096 demonstrations make an arena
   of 862,136 bytes on a 64-bit laptop, far below 2^32. The demonstrations
   alone are 786,432 bytes there, already past the ESP32-S3's 512 KB of
   internal memory, so nothing a board can hold is refused.

   Where size_t is 16 bits (every Arduino AVR board) the cap is 255, and
   there it describes the boards rather than an overflow: an int count
   cannot wrap a 32-bit product, and a shape whose arena passes 65,535 bytes
   is refused (see IRIS_ARENA). With avr-gcc 7.3.0 for the atmega2560 the
   smallest shape, IRIS_ARENA(1, 8, 1, 255), is 5,580 bytes, more than an
   Uno's 2 KB of memory and most of a Mega's 8 KB, and 255 takes is more
   than anyone records by hand. */
...
/* Every product is computed in unsigned long, which C guarantees is at
   least 32 bits, and not in size_t.

   Where size_t is 16 bits (every Arduino AVR board) products taken in
   size_t wrap, and iris_size would wrap identically, so iris_init would
   compare a wrapped need against an equally wrapped array size and could
   not refuse: computed in size_t, IRIS_ARENA(24,63,16,254) comes out as 88
   bytes for an instrument that needs 65,624, and the first loop of
   iris_reseed writes 6,048 bytes into those 88 (avr-gcc for the
   atmega328p). Computed wide, the true number appears, and in C an array
   that large is refused by the compiler: a build error naming the array.

   In C++, which is how Arduino compiles a .ino, it is a refusal at run time
   instead. An array size in C++ converts to std::size_t, 16 bits on AVR, so
   ...
   `need == 0` test in iris_init returns 0, and the sketch gets a null
   pointer, which every example checks. The failure is loud, later than it
   should be, and never a write past the array. */
...
/* --------------------------------------------------------------------------
   ...
   A guard never changes anything silently: whatever it does is reported
...
typedef enum {
  ...
  IRIS_TRAINING_DIVERGED = 1,  /* a run pushed a weight or bias past
                                ...
                                reports a fit and iris_is_trained is 1), from
                                weights that stopped where the guard stopped
                                them. Every trainer that continues from the
                                current weights then refuses: see
  ...
  IRIS_NAN_TRAPPED       = 2,  /* a not-a-number or an infinity was caught by
                                the call that set this, and contained: a
                                reading refused at iris_record's door, a
                                setting refused by a setter, a played output
                                replaced by the centre of the demonstrated
                                range, a training run that met one partway and
                                re-seeded to a finite, unfitted start, or a
                                trainer or diagnostic (PART 8, closed-form
                                included) that found one in a stored
                                demonstration and refused. That refusal writes
                                this status and nothing else; the
                                demonstration stays in the store, where it can
                                be found and deleted.                        */
  IRIS_RIDGE_ESCALATED   = 3,  /* a closed-form solve (ELM) needed its ridge
                                doubled to factor. The result is valid; the
                                data was harder than usual.                  */
  ...
  IRIS_DIVERGED_STUCK    = 5,  /* a warm trainer (iris_continue or
                                iris_continue_to_plateau) refused, because a
                                weight or bias sits exactly on ±IRIS_W_LIMIT,
                                where a divergence left it. They refuse on
                                every call while that is true, whatever else
                                has touched the status since. The way out is
                                iris_train (or iris_train_begin), which starts
                                over from the instrument's own seed: the
                                demonstrations are intact, only the weights are
                                damaged. That run is deterministic, so if the
                                same demonstrations diverge from the same seed
...
/* Any momentum velocity smaller than this is musically and numerically dead:
   ...
   host and a board disagree. The ESP32-S3's floating-point unit does not
   flush subnormals (board_probe's subnormal check, board log
   docs/board/2026-09-25-es3c28p.txt). 1e-30 is about eight powers of ten
   above the smallest normal float (about 1.2e-38), so every processor
...
/* The weight limit. A weight or bias past it means training is running
   away: the guard in PART 8 clamps it to exactly this value, reports
   IRIS_TRAINING_DIVERGED and stops the run, and a trainer that would
   continue from a weight sitting on it refuses with IRIS_DIVERGED_STUCK.
   iris_internal_tanh is exactly ±1 beyond |s| = 3, so one weight of 16 on
   its own saturates its hidden unit whenever its input is more than 3/16 of
   the way from the centre of its range to either end.
   ...
   against 40, 0 and 0 reports on the default fits. The golden training hash
   in tests/audit.c is the same at all three limits: no healthy reference run
   comes near it.
...
/* ==========================================================================
   PART 1 -- Mathematics the file provides itself

   A neural network needs a few mathematical functions: a squashing
   function, which bends each unit's sum into a curve, a square root, and a
   source of random numbers for the starting weights. This file cannot call
   math.h, so all three are written here, in a way that gives the same bits
   on every processor. The squashing function is the hot one: with 2 inputs,
   12 hidden units and 3 outputs the network calls it 15 times per
...
/* The nonlinearity, and why it is frozen.
   ...
   here as a sum near 1e20, where both halves of the ratio overflow to
   infinity and infinity over infinity is not-a-number, so every hidden unit
   would be not-a-number. With the test that reading plays an ordinary
   output with a healthy status. */
...
/* The square root, in integers, correctly rounded.
   ...
   error variable, can be set. A call is an undefined symbol in a
   freestanding build, and it makes the answer belong to whichever C library
   is linked. This function needs nothing and gives the same bits on every
   target.
   ...
   come out one at a time, from the top, 25 of them. With q the root found
   so far and b the next bit to try, setting b grows the square from q*q to
   (q+b)*(q+b), an increase of 2*q*b + b*b, so the bit is kept exactly when
   the remainder (the number minus q*q) can pay for it. The loop stores the
   remainder divided by b, which makes the price 2*q + b, and moving on to
   the next bit, half as big, doubles the stored remainder. Every quantity
   stays below 2^27, so 32-bit integers suffice.
   ...
   Special values follow IEEE 754: sqrt(+0) = +0, sqrt(-0) = -0,
   sqrt(+infinity) = +infinity, a NaN comes back as the same NaN made quiet,
   and any other negative input gives NaN. None of the four places passes a
   negative number: the Cholesky step checks that its diagonal is positive
   first, and the others take the root of a count or of a sum of squares.

   The price is speed: about 50 nanoseconds a call on the development
   laptop, where the hardware instruction takes about 6. That adds about 45
   nanoseconds to iris_novelty, 60 to iris_reseed, and 0.6 of the 5.0
...
/* xorshift32: a tiny, fast, repeatable random number generator.
   ...
   sequence, so the same seed always produces the same instrument, and
   "reroll" is a control you can go back on. Each step mixes the 32-bit
   state with shifted copies of itself; a state of 0 would stay 0 for ever,
   which is why iris_reseed takes a seed of 0 as 1. From any other state the
   step never reaches 0, so the replacement below of a 0 result by
   0x9E3779B9 (2^32 divided by the golden ratio) is never taken by an
   instrument this file made; it keeps a zeroed generator handed in from
   outside from returning 0 for ever. */
...
/* ==========================================================================
   ...
   An instrument is one structure: the network's weights, the ranges it
   measured, the demonstrations, and the bookkeeping of training. The
   structure itself is small; every array it points to is carved out of the
   arena you gave iris_init (PART 3), so the whole instrument sits in memory
   you own and can see.
   ========================================================================== */
...
struct iris {
  ...
  /* --- the network -------------------------------------------------------
     One hidden layer. Wekinator's default network has this shape, and with
     ten or twenty training examples a deeper network has far more capacity
     than data and memorises noise. One layer with a modest number of units
     is the right size for the information a musician gives it.

     Weights are stored flat and row-major, w1[h*n_in + i], so that the
     inner loop walks straight through memory rather than chasing pointers
     from layer objects to node objects. */
  ...
  /* --- normalisation -----------------------------------------------------
     ...
     millimetres; an accelerometer reads -2 to +2 g. Fed in raw, the network
     spends its effort on the big number and effectively ignores the small
     one, so the instrument records the range of everything it has seen and
     rescales before a value touches a weight (PART 5). */
  float *in_lo, *in_hi, *out_lo, *out_hi;

  /* --- the examples ------------------------------------------------------
     The training examples are not scratch data thrown away after training.
     They are the instrument: you must be able to look at them, hear them,
     and delete the bad one. */
  ...
  /* --- which demonstration is fighting the others (PART 8f) --------------
     ...
     every epoch of the last training session, not the final residual (PART
     8f says why the final residual is worthless once you train to
     convergence). A closed-form solve (PART 8d) has no epochs; it stores
     each example's squared miss under the solve, with res_epochs 1. */
  float   *ex_res;
  ...
  int32_t fitted;          /* this instrument has ever produced a fit.
                              ...
                              `trained` (the fit is stale) but not this (the
                              instrument still plays). iris_predict guards
                              on this one.                                  */
  float   last_error;
...
/* What the status does and does not cover.
   ...
   It reports numerical health, and a full store: a poisoned value trapped
   at the door or before an output, a diverged or stuck run, a ridge
   escalation, a prediction from an instrument that was never fitted, a
   closed-form solve that collapsed to a constant. Those are the conditions
   ...
   come back through the return value and leave the status alone, so that
   one out-of-range query cannot leave a polled screen showing a fault for
   ever. Check the return value of the call you made, and the status for
   whether the instrument itself is in trouble. */

/* Threading, in one sentence: never touch the same instrument from two
   places at once.
   ...
   thread across as many cores as you have; one instrument used only inside
   an interrupt, subject to the platform caveat below.
   ...
   The platform caveat decides the interrupt case on the board this library
   usually runs on. Everything above is about this code, not your chip. On
   an ESP32 under FreeRTOS, the real-time operating system its Arduino core
   runs on, the floating-point registers are not saved when an interrupt is
   taken (the core's configuration leaves CONFIG_FREERTOS_FPU_IN_ISR unset),
   so float arithmetic inside an interrupt handler, iris's or anyone's, can
   corrupt the interrupted task. iris is float throughout, so on that
   platform call no iris_ function from an interrupt handler: read the
   sensor there, set a flag, and call iris from the main loop.

   Timing, for the audio case. One prediction of a 2-12-3 instrument takes
   14.95 microseconds on the ES3C28P (ESP32-S3, 240 MHz): the median over
   repeated batches, with 99.9% of single calls within 19.8 and the worst at
   47 (board_probe, board log docs/board/2026-09-25-es3c28p.txt). Against
   the 20.8 microseconds of one audio sample at 48 kHz that is 1.4 times of
   margin, 72% of a core spent on playing alone; at a control rate of 1,000
   predictions a second it is 1.5% of a core. On the development laptop one
   prediction takes about 0.04 microseconds (`sh build.sh audit`). Training
   does not fit inside an audio callback: train in slices from the main
   loop (iris_train_slice), and never from an interrupt. */

/* How every function in this file reports failure: two rules, and before
   them the one line that answers "did it train?":
   ...
   The two rules below are each sound, but they meet badly in C, and
   `if (trainer(...))` is wrong in both directions depending on which
   trainer you called. The four trainers on the 20 demonstrations of the
   reference task in tests/audit.c (2-12-3, seed 1234, each from a fresh
   reseed, the closed-form one at lam0 1e-4; the refusal is an empty store):
   ...
   true. One more path returns -1 without being a refusal: a gradient run
   that meets a not-a-number partway (demonstrations spread wider than the
   largest float can span, say) has lost its fit, so it resets the
   instrument to its seed's starting weights, unfitted, sets
   IRIS_NAN_TRAPPED and returns -1 (iris_train returns 0). iris_is_trained
   reads one flag that every trainer sets on success and none sets
   otherwise, so it gives the same answer whichever trainer you called. It
   answers whether the fit matches the demonstrations stored now: a call
   ...
   iris_continue_to_plateau, iris_train_elm, iris_loo_error,
   iris_suggest_smoothing, iris_worst_example, iris_worst_example_id,
   iris_index_of, iris_id_at, iris_classify_1nn, iris_novelty and
   iris_example_stress. These cannot use rule 1 because 0 is often a good
   answer: a training error of 0 is a perfect fit, no ridge doublings is the
   best a solve can do, and a novelty of 0 means the reading is exactly on a
   demonstration.

   Where the rules differ for similar questions:
     - A position with no demonstration: iris_get answers 0 (rule 1: it
       ...
       (rule 2).
     - iris_train_elm's best outcome, 0 ridge doublings, reads as false.
   ...
   instrument that is unwell, and fail on a perfectly good one.
   ---------------------------------------------------------------------- */
...
/* ==========================================================================
   ...
   Building an instrument means checking that the shape you asked for makes
   sense, checking that the memory you gave is big enough, carving that
   memory into the arrays of PART 2, and drawing the random starting weights
   from the seed. This part also holds the training settings: the learning
   rate and momentum, which a musician never touches, and smoothing, which
   they may.
   ========================================================================== */

/* Bytes an instrument of this shape needs, or 0 for a shape that cannot be
   sized safely: any dimension out of range (cap above IRIS_MAX_EX included,
   see the note there), or an arena larger than this machine's size_t can
   hold, which on an AVR board is anything past 65,535 bytes.

   0 is a sentinel and does not protect you on its own: size_t is unsigned,
   so `bytes < iris_size(...)` is false when iris_size returns 0, and a
   caller using that idiom alone would sail past a bad shape. iris_init does
   not use it (see the bound inside iris_init); any other caller must test
   for 0 explicitly. */
/* Bytes a shape occupies: IRIS_ARENA evaluated at run time, so the macro and
   ...
   unsigned long). 0 when the total does not fit a size_t on this machine.
   No opinion on whether the shape is sensible; that is iris_size. iris_init
...
/* Randomise the weights: the reroll.
   ...
   learning before it starts. Scaling each weight by one over the square
   root of the number of inputs to its unit keeps the sums in the
   responsive part of the curve.
   ...
   closed-form trainer (PART 8d). The new starting weights replace the old
   ones at once, so if the trainer then refuses (no demonstrations, say) the
   instrument is left unfitted and plays as one (IRIS_NOT_FITTED) until a
   trainer succeeds.
...
IRIS_API iris *iris_init(void *mem, size_t bytes, int n_in, int n_hid, int n_out,
  ...
  /* Filled here, so the shuffle buffer never holds memory nobody wrote: the
     ...
     reached by a record made during a sliced run would be a crash on a
     dirty arena. The trainer also refills order[] at the start of every run
     and whenever the demonstration count changes under a running slice, so
...
/* Internal: the learning rate and momentum are not part of the interface,
   ...
   refuses the instrument with IRIS_DIVERGED_STUCK until iris_train starts
   it over.
   ...
   It stays as an internal hook: the tests use it to force divergences and
   wild settings (tests/audit.c, tests/train.c) and to check that a load
   puts the defaults back (tests/load.c). The clamps allow Weka's own pair,
   0.3 and 0.2. */
...
/* Smoothing: the one quality knob, in the musician's own terms.
   ...
   What it buys, and what it costs. Measured with iris_train on six
...
/* ==========================================================================
   ...
   The demonstrations are what the instrument learns from, so you must be
   able to add them, look at them and delete them. Deleting one
   demonstration is a five-line function, and it is what makes a mistimed
   button press cost one take rather than twenty minutes of work.
...
/* Lengths: unchecked, as everywhere (see the interface block). Reads exactly
   n_in floats from `in` and n_out from `out`. */
IRIS_API int iris_record(iris *k, const float *in, const float *out) { if (!k) return 0;
  ...
  /* A take was just stored, so the store is not full: clear IRIS_STORE_FULL,
     or `if (iris_get_status(k))` would answer "something is wrong" for the
     rest of the instrument's life after one full store. Only that status:
     other calls raise IRIS_NAN_TRAPPED too, and storing one good number
     does not disprove a not-a-number that training trapped. The next
     training run clears the status, which is when the instrument is known
     to be well again. */
...
/* Lengths: unchecked, as everywhere (see the interface block). Writes
   exactly n_in floats into `in` and n_out floats into `out`. Either may be
   null if you do not want that half. */
...
/* Delete whichever demonstration is closest to where you are standing now.
   ...
   "Closest" is measured exactly as iris_classify_1nn measures it, each
   ...
   picks the second take, while in fractions of each range they are 0.36
   and 1.16 and the first take is the one you are standing on.
...
IRIS_API void iris_clear(iris *k) {
  ...
  /* Every demonstration goes, and with them the fit: the instrument plays 0
     until it is trained again. Any sliced run in flight ends too. The
     trainer returns at once when there are no demonstrations, so without
     this a run would never advance or finish, and the loop
         while (iris_train_slice(k, 500)) { draw(); poll(); }
     would never end. */
  k->tr_running = 0; k->tr_done = 0; k->tr_n_ex = 0; k->tr_ref = 0.0f; k->tr_err = 0.0f;
  /* The worst-demonstration ledger (PART 8f) goes with the demonstrations
     it describes. It is indexed by position, so without this the takes
     recorded next would read the cleared takes' sums until the next
     training run: in tests/train.c, twelve clean takes recorded after a
     clear inherit a bad take's sum, and iris_worst_example names a clean one
     with a margin of 3.45, past IRIS_STRESS_FLAG. Emptied, the ledger's
     readers answer -1, as on a fresh instrument, and with the ceiling at 0
     as well iris_train_progress reads 0.0. */
...
/* ==========================================================================
   ...
   Sensors and sounds come in any units. Before training, every input and
   output is mapped onto a fixed scale using the range the demonstrations
   covered, so that no quantity outweighs another because of its units, and
   the network's answers are mapped back so that you hear your own units.

   Outputs go to 0.1-0.9 rather than 0-1 on purpose. The output layer uses a
   sigmoid. A true logistic only approaches 0 and 1, so a target of exactly
   1 would push a weight toward infinity for ever. This sigmoid, built on
   the clamped rational of PART 1, does reach 1 (iris_internal_sigmoid(6.0f)
   is 1.0f exactly), but the training signal through it, y*(1-y), is 0
   there, so a target at either end would still drive the output into the
   flat region where it stops learning. Headroom at both ends lets the
   network arrive.
   ...
   0.1/0.9 gives 1.85 times less gradient at the targets (iris-studies S03).
   It stays because it is part of what a saved instrument plays (PART 9),
   not because it measured better; it did not.
   ========================================================================== */
...
/* The ranges. Every input and every output gets the smallest and largest
   value the demonstrations gave it. Every trainer calls this before it
   starts; the neighbour functions (PART 10), iris_delete_nearest and
   iris_novelty call it on an instrument that has never been fitted. With
   no demonstrations it leaves the ranges as they are.

   An input that never moved is ignored. A switch left in one position, a
   sensor resting against its rail, a light sensor under steady light: an
   input that read the same in every demonstration tells the instrument
   nothing about what you want, and dividing by its width would turn the
   smallest wobble at play time into an enormous number. So:
   ...
   is fewer than 170 steps of the float's own resolution: a range that
   narrow is rounding and sensor noise, not a gesture. The 1e-6 covers
   inputs resting near zero, where a relative test alone would demand an
   exact zero.

   Why ignore it rather than give it a small width: a small width magnifies
   any movement at play time. An input held at 500 in every demonstration
   and given a width of 0.005 normalises to about 400 when it reads 501,
   where the demonstrations taught the network only [-1,+1]. With such a
   floor in place of this rule, on the six demonstrations tests/playing.c
   uses, a sweep of the other input spans 9.97 of the 10 demonstrated with
   the still input at 500, and 0.0000 with it at 501: every hidden unit
   saturates and the instrument becomes a constant.

   An output that never moved keeps a small nonzero width, because the
   network is trained toward it and the output scaling divides by the width:
   ...
   The floor is relative because an absolute 1e-6 added to a value above 32
   changes nothing in a float, whose representable numbers are already
   further apart there, so the width would stay zero and every prediction
   would be not-a-number: an absolute floor works up to 31.77 and fails from
   32.72. The widening goes downward only at the top of the float range: an
   output shown nothing below about 3.40279e38 would otherwise get hi =
   infinity, and the output scaling, the substitute a playing function
   writes (the centre of the range) and the saved file would all hold an
   infinity. Widened downward, both ends stay finite and lo < hi
   (tests/playing.c trains one with each trainer at the largest float).

   A limit that is not guarded: a width is a float, so demonstrations that
   span more than the largest float, about 3.4e38 end to end, give a width
   that overflows to infinity. Normalising a demonstration at either end of
   such a range gives not-a-number, so training traps it partway and leaves
   the instrument reseeded and unfitted (two inputs, one demonstrated at
   -2e38 and 2e38: iris_train returns 0 with IRIS_NAN_TRAPPED), and
   iris_save refuses an instrument holding such a range (PART 9). No sensor
   reads numbers of that size. */
...
/* The input scaling: each input maps to [-1,+1] across the range the
   ...
   does with normalizeAttributes on, the setting Wekinator ships. On the
   8-output reference task at 600 epochs, against the same network fed [0,1]
   inputs: training mean squared error 5.94e-4 -> 6.38e-5 (9.3x), grid
   root-mean-square error 0.0129 -> 0.0084 (1.54x)
   (docs/adr/0018-the-input-scaling-travels-with-the-file.md; iris-studies
   S12).

   A still input (zero width, see iris_internal_fit_ranges) maps to 0 for
   every finite reading. It is written v - v rather than 0 so that a reading
   which is not finite (a disconnected or broken sensor) still comes out as
...
/* ==========================================================================
   PART 6 -- Forward pass: playing the instrument
   ...
   step across the input space. A dozen of them, each with its step in a
   different place and direction, let the output bend into the shape you
   demonstrated, and training (PART 8) is the search for weights that put
   the steps in the right places.

   iris_predict wraps the network in the scaling of PART 5:
   ...
   squashing functions, one in each output's scaling back); on the ESP32-S3
   each division is a call to a library routine (the masthead). One
   prediction takes 14.95 microseconds there (the timing note above
   iris_get_status) and about 0.04 microseconds on the development laptop.
   Training the same instrument takes seconds, so playing is the cheap half.
   ========================================================================== */
...
/* Does this instrument fit this translation unit's working arrays?
   ...
   and that is a per-translation-unit setting: define IRIS_MAX_IN 4 in one
   .c file and not in another, and the two disagree about how big those
   arrays are while sharing one instrument through a pointer. iris_init
   checks the shape against the maxima of the unit that calls it, which may
   be the one with the large maxima; the unit with the small maxima then
   writes n_in floats into its own float x[4] (under AddressSanitizer:
   "stack-buffer-overflow, WRITE of size 4, [32,48) 'x.i'"). So the playing
   functions, the neighbour search and the trainers ask this first: two
   comparisons that turn a memory overwrite into an ordinary refusal. */
...
IRIS_API void iris_predict(iris *k, const float *in, float *out) { if (!k) return;
  ...
  /* Playing an instrument that was never fitted. Without this, the forward
     ...
     plausible-looking numbers with no symptom anywhere, so it plays the
     substitute above instead and reports IRIS_NOT_FITTED.

     It guards on `fitted`, not `trained`. iris_record and the delete
     functions clear `trained` (the fit no longer reflects the current
     example set), but the instrument must keep playing mid-performance
     (tests/playing.c holds that). `fitted` says "this has ever produced a
     fit"; iris_reseed and iris_clear clear it, as does loading a file saved
     before any fit. */
  if (!k->fitted) {
    for (int o = 0; o < k->n_out; ++o) out[o] = iris_internal_centre(k, o);
    k->status = IRIS_NOT_FITTED;
...
/* ==========================================================================
   ...
   Novelty is the distance from the current gesture to the nearest thing you
   demonstrated: 0 means exactly on an example. It lets the instrument know
   when it is improvising rather than recalling, which you can map to
   anything you like: noise, detuning, a light. It costs one pass over the
   demonstrations.

   What 1 means, precisely, because the obvious reading is wrong. The
   distance is taken between normalised inputs, which run from -1 to +1
   across each demonstrated range (PART 5), and divided by sqrt(n_in)/2, a
   constant that depends only on how many sensors you have, not on how far
   apart your demonstrations are. The normalised box has side 2 and
   diagonal 2*sqrt(n_in), so 1 means a quarter of that diagonal away from
   the nearest example: a quarter of every sensor's demonstrated range, in
   every sensor at once, and anything further also reads 1. An input that
   never moved adds nothing to the distance but still counts in n_in.

   So the usable range depends on how many takes you recorded. With four
   corner demonstrations (examples/00_minimal.c), 60.7% of the gesture
   square reads exactly 1.0 (on a 1001 x 1001 grid of probes), including the
   middle of the demonstrated space: "between your four takes" and "ten
   times outside them" read the same. With twenty-five demonstrations on a
   5 x 5 grid it never exceeds 0.5, reached at the centre of each cell, and
   two instruments are not comparable. A scale relative to the
   demonstrations' own spacing would fix that, and would change what every
   sketch that maps novelty hears; the scale is fixed.
   ...
   takes a non-const instrument. Without it, an instrument that has only
   been shown takes would measure in the 0..1 ranges iris_init starts with:
   with one input demonstrated at 0 and 1000, a reading of 10 would read 1
   before the first training run and 0.04 after it; it reads 0.04 in both.

   A reading that is not finite reads as 1, as far from home as it gets, and
   is answered before any fitting, so then iris_novelty writes nothing. An
   empty store reads 1 as well. It refuses with -1 (rule 2 of the failure
   rules above iris_get_status) for a null instrument and for a shape too
   big for this translation unit's working arrays (iris_internal_shape_fits).
   A distance is never negative, so a refusal cannot be mistaken for 0, a
   reading exactly on a take.
   ========================================================================== */
...
/* ==========================================================================
   ...
   the last demonstration seen always gets the final say.
   ========================================================================== */

/* Guard sweep, once per epoch: a not-a-number or infinity in any weight (or
   in the epoch's error) means the numbers are gone, so report it and
   ...
   stop. It passes over the weights once per epoch, where backpropagation
   passes over them once per demonstration, so it adds less than 1/n_ex to
   the work.                                                                */
#ifndef IRIS_NO_GUARDS
...
/* Is any weight or bias sitting exactly on the limit? That is the mark a
   divergence leaves: iris_internal_check_weights writes exactly
   ±IRIS_W_LIMIT into every weight it clamps, and only another training run
   can move it from there. A healthy fit ends strictly inside the limit (the
   largest weight in 2,264 healthy default fits was 15.9953, see the note on
...
/* --------------------------------------------------------------------------
   ...
   iris_train does not ask for an epoch count. It runs until the training
   error plateaus: every IRIS_CONV_WINDOW epochs it compares the error with
   the error one window earlier, and stops when the window bought less than
   IRIS_CONV_TOL of it. A short window (200-500 epochs) mistakes the
   ...
   What the plateau buys is recall. On the reference task of tests/audit.c
   ...
   and 19%). On noisy demonstrations a plateau run at smoothing 0 fits the
   ...
   show something true. Two ways in:
     ...
     - iris_train_begin, iris_train_slice and iris_train_progress run the
       same training as iris_train in slices, so a single-threaded sketch
       can draw a frame, read a touch and keep sound going between them. A
       sliced run is bit-identical to iris_train (see iris_train_begin).
   ...
   iris_continue is the fixed-epoch path: the per-demonstration update of
   Weka's MultilayerPerceptron, the network Wekinator ships, with a fixed
   epoch count. tests/audit.c pins its output to the bit.
   -------------------------------------------------------------------------- */
...
/* The ceiling has to fit the machine's int, because it is passed as one:
   ...
   (every Arduino AVR board), 60000 would become -5536 (avr-gcc for the
   atmega328p), the trainer's `epochs <= 0` guard would refuse, iris_train
   would return 0, and a sketch that ignored the return value would play an
   instrument that was never fitted. 30000 is the largest round number that
   fits a signed 16-bit int, and an 8-bit AVR at 16 MHz does not reach
   30,000 epochs in a time anyone will wait for, so there the ceiling is not
   the binding constraint; being positive is. */
...
/* Can this store be trained on? Every trainer and diagnostic asks this
   ...
   refusal leaves the instrument playing exactly as before.

   Three conditions: the shape fits this translation unit's working arrays
   (iris_internal_shape_fits), there is at least one demonstration, and
   every stored number is finite, since a not-a-number would poison every
   weight in the first epoch.

   The first two failures are mistakes in how the call was made, reported by
   the return value alone, like every argument mistake. A stored number that
   is not finite is a matter of numerical health, so that refusal sets
   IRIS_NAN_TRAPPED, and writes nothing else.

   iris_record and iris_load refuse a non-finite number at the door, so this
   test is defence in depth, for a store that got one another way: a
   ...
   demonstration stays in the store where the musician can find and delete
   it. A -DIRIS_NO_GUARDS build compiles this test out too, and then nothing
   stops a not-a-number reaching the weights. */
...
/* The refusal convention: a train call that did no training returns -1.0f
   and leaves `trained` alone, so a caller reading only the return value can
   tell a refusal from a repeat of the previous run. A run that meets a
   not-a-number partway returns -1.0f too, having reset the instrument to its
   seed's unfitted start (iris_internal_trap_nan). */
IRIS_API float iris_internal_train_run(iris *k, int epochs, int conv, int resume,
  ...
  /* Refuse before writing anything else. epochs <= 0 is "do nothing", not
     ...
     network as trained with a perfect fit. A poisoned demonstration sets
     IRIS_NAN_TRAPPED inside iris_internal_trainable.

     A slice's refusal also ends its run: a run in flight that finds nothing
     it can train on is over, however it got that way (the delete functions
     can empty a store as surely as iris_clear does). Ending it here covers
     every caller; without it the slice loop would spin for ever with the
     progress bar frozen. */
...
#ifndef IRIS_NO_GUARDS
  /* The divergence trap. When a run diverges, iris_internal_check_weights
     clamps the offending weights to exactly ±IRIS_W_LIMIT and stops. A run
     that continues from those weights starts with them on the clamp: epoch
     1 pushes one past, the guard fires again, and training stops after a
     single epoch. On the demonstrations of examples/02_fix_a_mistake.c, 14
     good ones plus one contradictory take diverge and pin one weight of 60;
     without this refusal, a warm run after the bad take is deleted does
     exactly 1 epoch per call, diverges again, and returns an
     ordinary-looking error each time, while the first output creeps 0.087,
     0.091, 0.106 over three calls against the 0.618 it played before the
     take. Zeroing the momentum does not help: it is the pinned weight, not
     the velocity.

     The demonstrations are fine; the weights are damaged, and refitting
     from the seed recovers the instrument (on the same demonstrations
     iris_train plays 0.618 again). So a run that would continue from pinned
     weights refuses with IRIS_DIVERGED_STUCK rather than pretending to
     train. It does not reseed on its own: a warm trainer is asked to keep
     the performer's weights, and replacing them silently would hand the
     performer a different instrument (see the warm trainers below).

     The weights decide, not the status (iris_internal_pinned), so the
     refusal holds on every call while a weight sits on the limit, and a
     status overwritten by an unrelated call cannot let a warm run through.
     iris_train and iris_train_begin reseed before their first epoch, so
     they never meet this test with pinned weights, and a sliced run ends
     itself if it diverges: in practice what this refuses are the warm
     trainers. The one write is the status, plus ending the run if a slice
     is refused. */
  ...
  if (!resume) {
    ...
    /* The data changed under a running slice: a record or a delete between
       ...
       leaves the count as it was (delete a bad take and record its
       replacement, the repair this library teaches) is seen too. The
       ...
       Also restart the plateau window. New data makes the error jump up,
       and a stale reference would read that rise as a plateau and end the
       run on the exact epoch the student added something.

       tr_done is not reset: the epoch budget stays monotonic, so a caller
       who records between every slice still reaches the ceiling instead of
       training for ever. */
    for (int i = 0; i < k->n_ex; ++i) k->order[i] = i;
    k->tr_n_ex = k->n_ex;
    k->tr_ref  = 0.0f;
  ...
  for (int ep = 0; ep < epochs; ++ep) {
    ...
    for (int s = 0; s < k->n_ex; ++s) {
      ...
      /* --- output layer error ---------------------------------------------
         This is a surrogate gradient, not the gradient.

         d_out = (predicted - target) * y*(1-y). y*(1-y) is the exact
         derivative of the true logistic, but iris_internal_sigmoid is built
         from iris_internal_tanh, the clamped rational of PART 1, so the
         backward pass is not the derivative of the forward pass. It is a
         surrogate, close enough in shape to point downhill.

         How far off: exact only at zero, and under-scaled by up to 2 times
         across the ordinary operating range (iris-studies S03 tabulates the
         ratio for the rational without its clamp, where it also changes
         ...
         reaches. Every repair tried measured worse (iris-studies S04), and
         the exact derivative of the rational makes no measurable difference
         (PART 1).

         What is traded away. A cross-entropy gradient (the gradient of the
         logarithmic loss usually paired with a sigmoid output: the miss
         alone, with no y*(1-y) factor) with the targets left in [0.1, 0.9]
         wins 32 of 32 seeds at 20 demonstrations by about 5%, and about 12%
         at a tuned learning rate. It loses at 10 demonstrations (1.094
         times worse), the regime a musician demonstrates in, and it widens
         the reroll spread in the undemonstrated gaps by 1.6 times. A reroll
         that changes the instrument is one of this library's promises, and
         y*(1-y) is the brake that keeps it steady at the demonstrations: a
         judgement about the use case made on top of a measurement
         (iris-studies S03, S04). */
      float rse = 0.0f;
      ...
      /* Each weight's velocity is 0.85 of the last one minus the learning
         rate times its gradient; the weight moves by its velocity. With
         weight decay, per weight and per demonstration visited:
             v = momentum * v - lr * gradient      (flushed)
             w = w + v
             w = w - wd * w                        (flushed)
         with wd = l2 * lr / n_ex, where l2 is 0.3 times the smoothing. The
         decay is taken after the velocity step, from the weight that step
         produced, and scaled by the learning rate as the gradient step is.
         At smoothing 0, `w[h] - 0.0f * w[h]` subtracts an exact zero, so the
         update is the undecayed one bit for bit, for one multiply and one
         subtraction per weight.
         ...
         zero, where a processor that flushes them and one that does not part
         ways. Six demonstrations with one still input at smoothing 1 (the
         check in tests/train.c) have a weight below IRIS_TINY after 2,225
         epochs and a subnormal one after 2,832 without the flush, and every
         such weight at exactly zero after 2,285 with it. A weight smaller
         than IRIS_TINY moves nothing, so zero loses nothing, and no healthy
         run at smoothing 0 has one: the golden hash does not move.

         Decoupled (applied to the weight, not folded into the gradient, so
         it does not build up in the momentum) and on weights only: a
         penalised bias shifts the function without reducing its freedom.
         Scaled by 1/n_ex so that a setting means the same however many
         demonstrations you have, as scikit-learn scales its penalty against
         the data. */
    ...
    /* The error floor: a fourth stopping rule, and at small demonstration
       counts usually the one that stops the run.

       tr_err is the mean squared error over every demonstration and
       output, in the network's output units (each output's demonstrated
       range spans the 0.8-wide band [0.1, 0.9]), added up during the epoch
       while the weights move; iris_last_error is measured after the run.
       So 1e-6 is a root-mean-square miss of 0.001 in those units, 0.125% of
       each output's range. It is absolute, the same whatever the data, the
       noise or the number of demonstrations, and was chosen as "close
       enough to a perfect fit", not derived.

       A network with a few dozen weights can fit a handful of
       ...
       It sits outside the `conv` guard (a perfect fit is a reason to stop
       on any path), so it also fires on the fixed-epoch path:
       iris_train_epochs_done says how many epochs ran. */
...
/* --------------------------------------------------------------------------
   ...
   Both carry on from the weights the instrument holds, with the momentum
   ...
   last run left; only a sliced run carries its order across slices. So n
   calls of iris_continue(k, 1) are not iris_continue(k, n): on the golden
   recipe of tests/audit.c, 800 one-epoch calls end with weights up to
   0.0182 away from one 800-epoch call.
   ...
   Sonami, NIME 2020). The tests/audit.c check "correction: parity fit,
   surgical drift" adds one take to a practised 20-take instrument and runs
   iris_continue(k, 20): the training root-mean-square error reaches
   0.0187, against 0.0181 for a cold 600-epoch retrain from the same seed,
   on a thirtieth of the epochs, and the mapping far from the new take
   moves 0.0025 on average, against 0.0033 for the retrain.

   The hazard. The weights remember every demonstration they were trained
   on, including one you have since deleted, and a warm run starts from
   exactly those weights, so continuing after a delete does not undo the
   take. On 20 demonstrations of a smooth target plus one contradictory
   ...
   After a warm run the seed alone does not describe the instrument, since
   the run drew its shuffle from the random state the last run left; the
   seed reproduces only a fresh iris_train. Replaying the same records,
   deletes and runs in the same order reproduces the instrument to the bit
   (the tests/audit.c check "event-sourced determinism"). A saved file
   carries the random state (PART 9) but not the momentum velocities, which
   a load sets to zero, so a warm run after a load matches the same run on
   the unsaved instrument only when its velocities were zero as well, and
   then to the bit (the check "save round trip carries the random state").

   Both refuse (-1) what every trainer refuses (iris_internal_trainable),
   and refuse with IRIS_DIVERGED_STUCK while a weight sits exactly on
   ±IRIS_W_LIMIT (the divergence trap in iris_internal_train_run); iris_train
   is the way out.
   -------------------------------------------------------------------------- */
...
/* The fixed-epoch trainer: exactly `epochs` more epochs from the current
   ...
   or -1 if it refused (a budget of zero or less included) or met a
   not-a-number partway, which leaves the instrument at its seed's unfitted
   start (iris_internal_trap_nan). After deleting a take, call iris_train,
   not this (the hazard above).

   It matches Weka MultilayerPerceptron's per-weight update recursion and
   its per-demonstration update granularity. The recursion is an exact
   algebraic rewrite of Weka's (ours: v = momentum*v - lr*g*x, w += v;
   theirs: delta = lr*err*x + momentum*delta_prev, w += delta; the same
   formula with the opposite sign convention, both starting at zero), with
   n_ex weight writes per epoch, not one.

   It is not numerically identical to Weka and cannot be: iris computes in
   binary32 and Weka in binary64 at every step. Nor is it identical in
   behaviour: iris's hidden units use the rational of PART 1 where Weka's
   use the logistic, its outputs are a sigmoid then a clamp where Weka's
   are linear and unclamped, its learning rate and momentum are 0.10 and
   0.85 where Weka's are 0.3 and 0.2, and it reshuffles every epoch where
   Weka shuffles once. Same rule, different quantities entering it,
   therefore different trajectories. Every bit-identical claim in this file
   is about this file's self-consistency (sliced against unsliced runs, save
   and load round trips, -O0 against -O3), never about Weka. The golden hash
   in tests/audit.c pins the weights this produces, so any change to it is a
...
/* Train until the training error plateaus, from the current weights: the
   ...
   if it refused or met a not-a-number partway (as iris_continue). After
   deleting a take, call iris_train, not this (the hazard above). */
...
/* What iris_train does before its first epoch, in this order; and
   ...
   The order matters: reseeding first would throw the playing instrument
   away and then refuse, leaving the musician with neither. */
...
/* Train. This is the one to call.
   ...
   them, reseeds from the instrument's own seed, then trains until the error
   stops improving. No epoch count to guess, no callback, no ceiling.

   It stops at whichever comes first:
     - the plateau test: every IRIS_CONV_WINDOW (2,000) epochs it compares
       the error with the error one window earlier, and stops when the
       window bought less than IRIS_CONV_TOL (10%) of it;
     - the ceiling, IRIS_CONV_CEILING (60,000 epochs; 30,000 where int is
       16 bits);
     - the error floor (the note at `tr_err < 1e-6f` in the engine), an
       absolute floor on the epoch's mean squared error, which at small
       demonstration counts is usually what stops the run;
     - the divergence guard: a weight past ±IRIS_W_LIMIT is clamped and the
       run ends there with status IRIS_TRAINING_DIVERGED.
   iris_train_epochs_done tells you how many epochs ran.

   Returns 1 if it fitted, 0 if not. A refusal (no demonstrations, a null
   instrument, a shape too big for this translation unit, or a
   demonstration that is not finite, which also sets IRIS_NAN_TRAPPED)
   touches neither the weights nor the ranges. A not-a-number met partway
   through a run also returns 0, with IRIS_NAN_TRAPPED, and leaves the
   instrument reseeded and unfitted. A run the divergence guard stopped
   returns 1: the instrument was fitted, and the status says how. How well
   it fits is iris_last_error(k).

   It never refuses a stuck instrument: starting over from the seed is the
   way out of IRIS_DIVERGED_STUCK, whose weights are the only damaged part.
   And it is the call to make after deleting a take, which then leaves
   nothing behind in the weights; the warm trainers would keep its
   influence (the hazard above). */
IRIS_API int iris_train(iris *k) {
  if (!k) return 0;
  /* Fit from a defined start, always: continuing would keep a deleted
     take's crater with a healthy status and a clean-looking training error
     (the hazard above). Continuing is what the warm trainers are for. */
  ...
  /* What the plateau rule bets on, and when the bet is wrong.
     ...
     stopping sooner the wrong one (a fixed 100 epochs beats it by 1.2 to
     2.2 times at noise 0.05 and above, unless smoothing is on): see
     ...
     The error sits on a false plateau from epoch 4,000 past 160,000 and
     then falls two orders of magnitude (seeds 7, 42 and 12345 do the same;
     seed 1234 stays on it until after 320,000). The plateau outlasts the
     maximum budget, so raising `ceiling` on iris_continue_to_plateau cannot
     help: a ceiling is a maximum, and the run stops far below it. If your
     demonstrations are clean and recall matters more to you than what
     happens between takes, the escape is iris_continue(k, 400000) after
     this: a real choice with a real cost, so it is yours to make. */
...
/* The training error of the current weights: the mean squared error over
   every stored demonstration and output, in the network's output units,
   measured with one forward pass per demonstration when a trainer finishes
   (a gradient run or slice, or a closed-form solve) and when a file is
   loaded (iris_internal_recall_error). So the same weights and
   demonstrations read the same bits however they arrived. 1.0 before any
   fit, after iris_clear and after a run that ended unfitted. A record or a
   delete does not re-measure it; a load does ("After a load", PART 9). */
...
/* Leave-one-out cross-validation: a real held-out error, at a size where
   you can afford it.

   Everything else in this library measures itself against the
   demonstrations it was fitted on, and training stops when training error
   plateaus, which is not the same event as "it got as good as it will get
   at things it has not seen". The usual remedy, holding back 20% as a
   validation set, is unavailable here: at 20 demonstrations that discards
   4 of them, and you cannot spare 4.

   Leave-one-out fits this regime. Hide one demonstration, refit on the
   rest, and see how far off the hidden one you land; do that once per
   demonstration and average. Nothing is discarded; every demonstration is
   used for training in every fold but its own.

   What it costs: n_ex + 1 fits of `epochs` each (600 by default), the
   folds and then the final refit. At 20 demonstrations of a 2-12-3
   instrument that is about 21 ms on the development laptop. It has not
   been timed on the board; an estimate from the board log's iris_continue
   figure (600 epochs of 20 demonstrations in 449 ms) puts it near 9 s
   there. It is a deliberate, occasional act, not something for a play
   loop.

   How to use it: for comparison, not as an absolute grade. Run it at
   several smoothing values and take the lowest, which picks a setting
   without tuning on the very numbers you then report.
   ...
   on the outputs, seed 1234. The left column is this function; the right
   is the mean squared error of the plateau-trained instrument against the
   clean target on a 21 x 21 grid:
   ...
   It ranks the ends correctly and picks a neighbour of the true best,
   which here is nearly as good: the known behaviour of leave-one-out at
   small n, nearly unbiased but noisy, with each fold training on n-1
   demonstrations. It also scores 600-epoch fits, not the plateau run you
   play. Treat it as a coarse ranking, whether to smooth and roughly how
   much. Its absolute value is not comparable to a held-out error (the
   columns above differ by three to eight times), so use it only to compare
   settings with each other.

   Every fold trains from the same seed, so the folds differ only by which
   demonstration was hidden. epochs <= 0 takes 600. Returns the mean
   squared error per output, in the demonstrations' own units, or -1 if it
   refused: fewer than 3 demonstrations, or a store no trainer would accept
   ...
   IRIS_NAN_TRAPPED only for a demonstration that is not finite, and it
   never returns a not-a-number.

   The instrument is left refitted on all demonstrations, from that same
   seed, with iris_continue for `epochs`: valid to play, but not the
   instrument you had, since it has been retrained for a fixed 600 epochs
   rather than to the plateau. Save first, or call iris_train afterwards, if
   that matters. */
...
/* Suggest a smoothing value: an explicit, occasional act, not an automatic
   one.
   ...
   instrument's seed, not the plateau run iris_train makes and you then
   play: a plateau fit costs about 25 times more, averaging 14,000 to
   ...
   fractions. iris_loo_error itself reports raw units. An output whose
   demonstrations never moved carries no evidence about smoothing and is
   left out.

   Why it is not automatic. On the same 36 datasets with 16 rerolls each:
     - It is not stable: rerolling the same demonstrations changed its
       answer, 2.17 distinct values per dataset on average, and an
       instrument whose smoothing changes when you reroll stops being
       predictable.
     - It is sometimes worse than doing nothing: 10.9% of its picks gave
       the plateau-trained instrument a worse held-out error than
       smoothing 0.
   ...
   setting would have gained (iris-studies S02). Treat the number as a
   suggestion to audition, and if you like where you land, pin it in your
   code rather than re-deriving it, so your instrument stays put.

   Asking for advice must not cost you your instrument. Every fold refits
   the network, so before the first one this copies every byte the
   instrument owns (weights, momentum velocities, ranges, demonstrations,
   the residual ledger, the random state, the progress counters, the
   status, `fitted` and `trained`) into your scratch, and copies it all
   back afterwards. A stale instrument that is still playing goes on
   playing, and a warm trainer called afterwards continues exactly as it
   would have. The arena is exactly sized and has nowhere to keep that
   copy, hence the scratch: at least IRIS_ARENA(n_in, n_hid, n_out, cap)
   bytes (what iris_size returns at run time), at any alignment, not
   overlapping the instrument. It refuses otherwise, and on anything
   iris_loo_error refuses, having changed nothing but, for a demonstration
   that is not finite, the status. A mistake in the arguments is found
   before the store is looked at, so it leaves the status alone even when a
   demonstration is poisoned too. */
...
/* ==========================================================================
   ...
   When an instrument feels wrong you usually have no idea which of your
   twenty demonstrations is wrong, so you re-record at random. This points
   at one: the take the network found hardest to reconcile with the rest.

   It is the integral, not the endpoint. The obvious detector is the final
   training residual: after training, ask each demonstration how badly the
   network still misses it. That gets worse as the mistake gets bigger,
   because given enough epochs the optimiser bends the surface far enough to
   fit the bad point too, after which it looks like every other point. So
   this sums each demonstration's squared error over every epoch instead,
   which measures how long it fought rather than where it ended up. A
   demonstration that agrees with its neighbours is fitted early and stays
   fitted; one that contradicts them stays wrong for thousands of epochs.
   That ranking is stable across training budgets where the endpoint is not.

   What you get back is a margin, not a level. The worst-of-n stress score
   rises with n on clean data with nothing wrong at all, so a screen wired
   to a fixed level would be silent on small instruments and cry wolf on
   large ones. Worst divided by second-worst does not drift.
   IRIS_STRESS_FLAG is 2.5.
   ...
   crowd, and at ten demonstrations the clean margin alone reaches 5.04
   (docs/adr/0019-the-residual-ledger-integrates-it-does-not-sample.md;
   iris-studies S10), which would be a false accusation.

   The limit that travels with it: every number below comes from one clean
   offset on one output of a smooth, noiseless truth. A 5% offset of that
   kind is smaller than the spread between two takes of the same human
   gesture, and on clean synthetic sessions iris_train still ranks it first
   (18 of 20, below), but real demonstrations are inconsistent in ways that
   are not one displaced output, and none of this has been checked against
   a recorded human gesture.

   The flag is not silent on clean sessions, and it can miss. Measured with
   `tests/elm.c measure` on the protocol of that decision record (12 hidden
   units, 8 outputs, 200 clean sessions at each of 20, 50 and 100
   demonstrations), the largest clean margin after iris_train is 3.23, 4.05
   and 3.22, and after the closed-form trainer (PART 8d) 9.99; the table in
   PART 8d gives how many clean sessions reach IRIS_STRESS_FLAG and how
   often an offset take is ranked first. So a flag is a take to listen to
   again, not a take to delete unheard, and no flag does not prove every
   take is good. Not built: for the closed-form trainer, the leave-one-out
   residual (each demonstration's miss divided by one minus its leverage,
   its diagonal entry of the hat matrix, which turns the demonstrated
   targets into the fitted ones) would come from the same Cholesky factor in
   about n times (nh+1) squared operations.

   Cost: sizeof(float) * cap in the arena (512 bytes at cap 128, 1 KB at cap
   256) and one float addition per demonstration per epoch.
   ...
   example with its gradient's effect summed over training) are in that
   decision record too.
   ========================================================================== */
...
/* The one to point at. Returns the index of the demonstration that fought
   ...
   The caller decides whether to speak, with the same condition in every
   sketch:
   ...
   Use iris_worst_example_id rather than iris_worst_example plus iris_id_at:
   it returns the identifier directly, and identifiers survive deletions
   where indices do not. Both stay public for callers who want the position.
...
/* ==========================================================================
   ...
   for the bias) and a target z for each output (the demonstrated output,
   mapped back through the sigmoid). The output weights beta that fit best
   in the least-squares sense solve the linear system
   ...
   epochs, no iteration. The literature calls this an extreme learning
   machine, and has argued for 20 years about whether it deserves a name of
   its own; it is here because it works here.

   Two measurements make it work in single precision on this network.

   Gain. The backpropagation starting weights (1/sqrt(n_in)) rely on
   training to grow. Frozen at that scale, tanh of a normalised input barely
   ...
   nh=48), so 2 is a good constant rather than the best one. It stays
   because a 1.2% gain on synthetic targets does not justify a change to
   training, which would re-pin the closed-form hashes in tests/elm.c.
   ...
   well-spread demonstrations (docs/adr/0009-ridge-is-mandatory.md;
   iris-studies S16). The ridge is relative (lam0 * trace/(nh+1), so it
   scales with the data) plus a floor of 1e-7, and it escalates
   deterministically: doubled on a failed factorisation, at most 8 times,
   ...
   the middle of its range, where an unpenalised one lets heavy smoothing
   settle near the average of what was demonstrated (averaged in logit
   units: the logit is the inverse of the sigmoid, the space the solve works
   in). Smoothing s is stored as weight decay 0.3 s (PART 3); the solve adds
   four times that, 1.2 s, to the diagonal entry of every output weight, on
   top of the relative ridge above. At smoothing 0 the added term is exactly
   zero, so the solve is bit-for-bit the unsmoothed one. The extra ridge is
   absolute, not relative to the data: like backpropagation's weight decay,
   its pull stays fixed while the evidence grows with every demonstration,
   so smoothing matters most with few takes.

   The four is measured, not derived. Carrying backpropagation's weight
   decay into logit units at the sigmoid's steepest slope (0.25) gives
   sixteen. With sixteen, the best setting for noisy demonstrations sat
   between 0.05 and 0.2 and smoothing 1 cost clean demonstrations 25% at 20
   takes; with four, the best setting sits between 0.1 and 1 and smoothing 1
   costs clean ones 8%. The table is `tests/elm.c measure` (the figures for
   sixteen come from the same program with the constant edited): nh 12,
   lam0 1e-4, 4 target shapes x 16 seeds, outputs spanning about 0..1. Each
   cell is two root-mean-square errors: at the demonstrations, then on
   fresh points against the clean target.
   ...
   Recall error rises at every step, which is what smoothing promises. On 8
   or 20 noisy demonstrations the held-out error at 0.3 falls by 10% to 40%,
   most with the fewest and noisiest takes; on 20 clean ones 0.3 costs
   nothing. On 50, where least squares already averages the noise away,
   smoothing only adds bias: 0.3 costs 11% on lightly noisy takes and 17% on
   clean ones. The default stays 0, for the reason PART 3 gives.

   Nothing changes unless the solve works. Every refusal (a bad argument,
   too little scratch, a poisoned demonstration, or a factorisation that
   fails even after escalation) leaves every byte of the instrument as it
   was, and the return value is the report; the poisoned demonstration also
   sets the status to IRIS_NAN_TRAPPED, as every trainer does (see
   iris_internal_trainable). That takes some care, because a solve needs the
   new ranges and the new frozen layer before it can know whether it will
   succeed:
   ...
   A solution containing a non-finite number is a failed solve too: it can
   only come from demonstrations so far apart that their span overflows a
   float.

   Which demonstration is fighting (PART 8f). A solve has no epochs to sum
   over, so it leaves each demonstration's squared miss under the solve in
   the ledger, in the same normalised units, with res_epochs 1, replacing
   whatever an earlier backpropagation run left there. After
   backpropagation the endpoint residual is worthless, because given enough
   epochs the optimiser bends onto the bad take; the ridged solve has only
   nh+1 numbers per output to bend with, so the endpoint still carries the
   signal, less of it than the integrated ledger does. `tests/elm.c
   measure`, on the protocol of
   docs/adr/0019-the-residual-ledger-integrates-it-does-not-sample.md
   (iris-studies S10; nh 12, 8 outputs, one take offset on one output; hits
   of 20 sessions, and clean sessions whose margin reaches IRIS_STRESS_FLAG,
   of 200):
   ...
   twice as often as after backpropagation: treat a flagged take as one to
   listen to again, not one to delete unheard.
   ...
   weights can lie beyond IRIS_W_LIMIT, which is legitimate here; the note
   at IRIS_W_LIMIT says what that means for gradient training afterwards.
   The solve targets logit space, the exact inverse of this file's sigmoid,
   so the forward pass lands on the normalised targets. That makes it a
   bounded-output variant of the backpropagation network's output layer,
   not an equivalent. docs/adr/0008-elm-same-network-better-math.md
   (iris-studies S15) compares it with a solve whose outputs are linear;
   nothing in this repository bounds the difference between the closed-form
   fit and a backpropagation fit.

   Reroll is the reason to love it: a new seed is a new frozen random layer,
   undiluted by any training, steady at the demonstrations and lively in the
   gaps (the tests/audit.c check "ELM: same seed, same bits; reroll
   character" measures the demonstrations moving 0.0106 between seeds and
   the gaps 0.1123). iris_reseed(k, new_seed) then iris_train_elm is the
   whole gesture: the purest form of "same demonstrations, different
   ...
   per width: docs/adr/0008-elm-same-network-better-math.md (iris-studies
   S15, S06).

   Determinism: the hidden layer is drawn from k->seed by a local random
   number generator (k->rng is never touched, so a solve does not move the
   random state a warm trainer draws from), accumulation order is fixed by
...
/* Inverse of iris_internal_sigmoid: the sum z with
...
/* Internal: the solve itself, with the hidden-layer gains as arguments. Use
   ...
   iris_train_elm and for the gain measurement (iris-studies S06).
...
#ifndef IRIS_NO_GUARDS
    /* Did it learn a mapping? A large enough lam0, or a zero gain_w, drives
       every output weight toward nothing, and the solve then maps every
       gesture to the same sound. That is not a failed solve by any
       numerical test: no value is bad and the ridge did what it was asked.
       So ask the question that matters to a musician: do the gestures they
       demonstrated still sound different? Everything is compared in
       normalised units, so the answer does not depend on the caller's
       units.
       ...
       themselves, not of the ranges, which iris_internal_fit_ranges widens
       for a constant output. An input that never moved reads 0 in every
       demonstration (PART 5), so it never counts as moving, and an
       instrument whose inputs all stood still is never reported.

       Collapsed means: on every output the demonstrations changed, the
       fitted outputs move less than half a percent as far as the
       demonstrations did, a constant with rounding on it. The status says
       IRIS_SOLVE_COLLAPSED, a report of its own: the solve is still
       installed, a smaller lam0 or a larger gain_w brings the mapping back,
       and no trainer refuses because of it. */
    {
...
/* ==========================================================================
   ...
   A trained instrument has to be a thing you can put somewhere and get
   back. iris_save turns it into a run of bytes you can store in flash or a
   file, and iris_load turns those bytes back into the same instrument. The
   file carries the weights and the demonstrations, so whoever receives it
   ...
   second kind. Each one refuses a value that would go wrong later if it
   were let in:
   ...
   After a load the instrument is the saved one, at rest: fitted and
   ...
   it reported before the save, to the bit. For a stale one (a take recorded
   or deleted since the last fit) it is not: the saved instrument reported
   the error over the demonstrations it was fitted on, and the load measures
   the same weights over the demonstrations the file holds.
   ...
   demonstrations stored with it. Record one more take after training and
   the instrument is fitted but not trained: it keeps playing. iris_save
   writes the two separately, so after a save and a load it still plays,
   the same bits as before, and iris_is_trained still says 0.
   ...
   changes while iris_save is copying it (a second thread, an interrupt, the
   sliced trainer driven from a timer), the buffer holds a mixture of two
   instruments, and the checksum, computed over the mixture, matches it by
   construction; if every value obeys the rules, it loads. One instrument
   belongs to one thread (see the threading note above iris_get_status); do
...
/* CRC-32, the 32-bit cyclic redundancy check of IEEE 802.3 (the one zip and
   Ethernet use), computed a bit at a time so there is no 1 KB table to
   carry onto a microcontroller. It runs only on save and load, never while
   playing: 5.5 microseconds for the 872-byte file of a 2-12-3 instrument
   with 20 demonstrations on the development laptop, about 6.5 nanoseconds a
   byte.
   ...
   every rule above, so the instrument loads and plays something subtly
   wrong with nothing reporting anything, and the musician assumes they
   mis-trained it. The checksum turns that into a refusal. */
...
/* Little-endian, one byte at a time. Casting the caller's buffer to a
   uint32_t or float pointer instead would demand 4-byte alignment the
   buffer need not have (undefined behaviour in C, and a fault on
   microcontrollers whose loads must be aligned) and would write the host's
   byte order into the file. Shifts and masks give the same bytes on every
   machine. */
...
/* Every rule in the table above, reading the file and writing nothing. It
   ...
   it: iris_load, before it touches the instrument, and iris_save, of the
   bytes it has just written. One copy of the rules, so the writer and the
   reader cannot come to disagree about what a valid file is. */
...
/* Writes the instrument into buf, in the order of the table above. Returns
   the number of bytes written, or 0 if buf is missing or smaller than
...
/* ==========================================================================
   ...
   The network is not the only way to answer a gesture. k-NN, "k nearest
   neighbours", finds the k demonstrations nearest to the gesture and blends
   what they said; 1-NN, the case k = 1, snaps to the single nearest one.
   The network of PARTS 6 and 8, a multilayer perceptron, is called the MLP
   below.
   ...
   These two functions play both modes with no training, no seed and no
   extra arena bytes: they are a weighted read of the demonstrations the
   instrument already carries.

     - It does not reroll. There is no seed and nothing random; the same
       demonstrations always give the same instrument, bit for bit. It is
       sampler-like where the MLP is morph-like: it plays back and blends
       your demonstrations.
     ...
     - Recall. Standing on a demonstration returns that demonstration: to
       ...
       exact-recall check in tests/audit.c measures a worst error of 6e-8
       on outputs between 0 and 1). Only another demonstration almost on
       top of it pulls the answer measurably away. The MLP does not quite
       get there: trained to its plateau it misses its own demonstrations
       by a root-mean-square 0.0012 on outputs whose demonstrated ranges
       are 0.45 to 0.8 wide, about 0.2% of the range (the convergence check
       in tests/audit.c).
     ...
     - Seams, on purpose. Between two demonstrations the output can step 31
       ...
       1.9 times (docs/adr/0010-knn-the-sampler-beside-the-morpher.md;
       iris-studies S14). That is the sampler character.
     ...
     - Structural safety. The answer is a weighted average of demonstrated
       ...
       never leaves the range you demonstrated, whatever finite reading
       comes in, however far from every take (a reading that is not finite
       is refused, with the substitute and IRIS_NAN_TRAPPED). When every
     ...
     - The accuracy floor. The MLP generalises better at every count of
       demonstrations measured (2.4 times at 5, 2.1 times at 200; the same
       decision record). Choose k-NN for its character, never for accuracy.
       For discrete outputs use iris_classify_1nn, which returns a stored
       label verbatim: a blend of labels is exactly a label only where all k
       neighbours agree, and a value between labels where they do not.
   ...
   fitted instrument keeps its ranges when you record or delete; train
   again to measure in the new ones (iris_internal_fit_ranges alone would
   also change what iris_predict plays, because the network was trained on
   the old ranges). Ties resolve to the earliest-recorded demonstration, the
   same rule as Weka's LinearNNSearch, the search under desktop Wekinator's
   classifier, so decisions are comparable. The Java program itself has not
...
/* The far distance, for when every ordinary distance has overflowed: the
   ...
   When it is needed. The ranges are the ones the instrument was last
   fitted to, and a take recorded since can lie any distance outside them:
   a take at 1e20 on an input fitted to 0..1 is 1e20 ranges from a reading
   of 0.5, and the square of that overflows to infinity. With every stored
   take that far out no ordinary distance is finite, so a finite reading
   would have no nearest take: iris_knn_predict would play its substitute,
   outside the outputs demonstrated, iris_classify_1nn would answer -1 and
   iris_delete_nearest would delete nothing, with takes stored. The
   neighbour functions scan again with this distance instead. Capped, each
   input adds at most 10^30, so between finite numbers it is always finite,
   and below 10^38 for any IRIS_MAX_IN up to 340,000. Skipping a still
   input keeps an overflowed difference times its zero scale from making a
   not-a-number.
   ...
   without, on the development laptop). Used only when no ordinary distance
   is finite, they cost nothing otherwise, and every reading with a finite
   distance to some take plays exactly what it did without them. */
...
/* k-NN inverse-squared-distance-weighted regression. k neighbours (default
   ...
   writes. */
/* Lengths: unchecked, as everywhere (see the interface block). Reads
   exactly n_in floats from `in` and writes exactly n_out into `out`. */
IRIS_API void iris_knn_predict(iris *k, const float *in, float *out, int kk) { if (!k) return;
  ...
  /* The neighbour count, clamped to at least 1 and at most n_ex and
     ...
     gcc that n_ex is at least 1 from here on, and the lower clamp comes
     last, which makes kk at least 1 whatever n_ex is. Either of those on its
     own, as does filling every slot below, clears gcc-15's "bi may be used
...
#endif
  /* The blend, written as the nearest neighbour's value plus the weighted
     mean of how far each neighbour's value lies from it:

         out = y0 + sum( s * (y - y0) ),    s = w / sum( w )

     where s is a neighbour's share of the total weight. That is the
     ordinary weighted mean, sum(w * y) / sum(w), rearranged, and the
     rearrangement makes it exact when every neighbour agrees: each
     difference is then exactly zero, so the answer is exactly y0. The
     ordinary form rounds: run through the checks in tests/playing.c, it
     fails to return the label on 291,862 of 364,140 queries on stores whose
     labels all agree (a label of 3 comes back as 2.9999998, which a C cast
     to int turns into class 2), and 44,504 of 800,000 random queries land a
     ...
     once two values differ by more than about 3e29; a share is at most 1,
     so s * (y - y0) is never larger than the difference it scales.
     ...
     that rounding cannot cross (0 of the 800,000, and 0 of 1,000,000
     queries on stores built from values at the very ends of their range).
     The answer is still held between the smallest and largest of the
     neighbours' values for the one case that argument does not cover: two
     values of opposite sign whose difference is larger than the largest
     float and overflows to infinity.

     The nearest neighbour's own term is s * (y0 - y0), which is zero, so
     the sum starts at the second slot. Fewer than kk rows can be in the
     slots when some distances are not finite; the blend uses the ones that
     are there. */
...
/* 1-NN classification: snap to the single nearest demonstration and return
   its outputs verbatim (bit for bit) plus its identifier, or -1 if the
   store is empty, the shape is too big for this translation unit, or the
   reading is not finite. For a classifier task store the class label in
   out[0]; this then follows the rules of desktop Wekinator's default for
   discrete outputs, Weka's IBk nearest-neighbour classifier with k=1:
   distance normalised by each input's range, Euclidean (straight-line),
   the first-recorded demonstration winning a tie. Like iris_knn_predict it
   writes the status and the ranges of a never-fitted instrument, so it
   takes a non-const one. */
/* Lengths: unchecked, as everywhere (see the interface block). Reads
   exactly n_in floats from `in` and writes exactly n_out into `out`; `out`
   may be null when only the identifier is wanted. */
...
#ifndef IRIS_NO_GUARDS
    /* Verbatim means verbatim for every healthy value, but a not-a-number
       stored in an example's outputs must not escape as a class label.
       Substitute and report; the returned identifier still names the row so
