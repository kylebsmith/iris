...
#   reference      tests/reference/run.py --quick, iris against a second
#                  implementation of its model in 64-bit floating point,
...
arm_reference() {
  if ! python3 -c 'import numpy, sklearn' > /dev/null 2>&1; then
    say "SKIP  reference: python3 cannot import numpy and scikit-learn (python3 -m pip install -r tests/reference/requirements.txt)"
    if [ -n "${CI:-}" ] || [ -n "${IRIS_REQUIRE_REFERENCE:-}" ]; then
      fail "reference: CI or IRIS_REQUIRE_REFERENCE is set, so a skip is a failure"
    fi
    return 0
  fi
